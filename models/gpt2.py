...
class GPT2Model(GPTPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    embeddings_dropped = self.embed_dropout(embeddings)

    return embeddings_dropped
  ...
  def hidden_state_to_token(self, hidden_state):
    ...
    """
    ### YOUR CODE HERE
    logits = hidden_state @ self.word_embedding.weight.transpose(-2, -1)
    
    return logits


  @classmethod
  def from_pretrained(cls, model='gpt2', d=768, l=12, num_heads=12, use_lora=False, r=None, alpha=None):
    ...
    our_model = GPT2Model(GPT2Config(hidden_size=d, num_hidden_layers=l,num_attention_heads=num_heads,
                                     intermediate_size=d*3, use_lora=use_lora, r=r, alpha=alpha)).eval()
